    ...
    try {
      const res = await fetch('/api/payment/mercado-pago', {
        ...
        body: JSON.stringify({
          ...
          price: billingCycle === 'monthly' ? 47.90 : 478.80
        })
      });
      ...
      <main className="pt-40 pb-20 px-6">
        <div className="max-w-7xl mx-auto space-y-16">
          <div className="text-center space-y-4">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-slate-800 dark:text-white">Planos que cabem na sua <span className="text-gradient-blue">ambição.</span></h1>
              ...
              <motion.div
                ...
                className={`relative p-10 rounded-[3rem] border-2 transition-all ${
                  plan.premium 
                    ...
                    : 'border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 text-slate-900 dark:text-white'
                }`}
              >
                {plan.badge && (
                ...
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 font-bold mt-2 text-sm">{plan.description}</p>
                  </div>
                  ...
                  <div className="space-y-4 py-8">
                    {plan.features.map((feature, i) => (
                      <div key={i} className="flex items-center gap-3">
                        <div className={`p-1 rounded-full ${plan.premium ? 'bg-blue-100 text-blue-600' : 'bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-300'}`}>
                  ...
                  <button 
                    ...
                    plan.premium 
                      ...
                      : 'bg-white dark:bg-slate-800 border-2 border-slate-100 dark:border-slate-700 text-slate-800 dark:text-white hover:bg-slate-50 dark:hover:bg-slate-700'
                  }`}>
          ...
          <div className="max-w-4xl mx-auto bg-slate-50 dark:bg-slate-900/50 p-12 rounded-[3rem] border border-slate-100 dark:border-slate-800 grid md:grid-cols-3 gap-8 text-center">
             <div className="space-y-2">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pais Ativos</p>
             </div>
             <div className="space-y-2">
                ...
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Lições Geradas</p>
             </div>
             <div className="space-y-2">
                <ShieldCheck className="mx-auto text-blue-600" size={32} />
                <p className="text-2xl font-black text-slate-800 dark:text-white">100%</p>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Seguro para Crianças</p>
             </div>
          </div>
        </div>
      </main>
